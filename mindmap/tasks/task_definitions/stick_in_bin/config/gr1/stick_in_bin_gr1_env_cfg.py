...
from mindmap.tasks.task_definitions.stick_in_bin.config.gr1 import mdp
from mindmap.tasks.task_definitions.stick_in_bin.stick_in_bin_env_cfg import StickInBinEnvCfg


def place_drum(target_side: TargetSide):
    if target_side == TargetSide.LEFT:
        origin = [5.1, 2.33, 0.0]
    elif target_side == TargetSide.RIGHT:
        origin = [4.08, 2.33, 0.0]
    else:
    ...
    return EventTerm(
        ...
        params={
            ...
            "min_separation": 0.1,
            "asset_cfgs": [SceneEntityCfg("open_drum")],
        },
    )


def randomize_pick_up_object_pose_event_term():
    return EventTerm(
        func=franka_stack_events.randomize_object_pose,
        mode="reset",
        params={
...
class EventTargetRightCfg:
    ...
    randomize_pick_up_object_pose = randomize_pick_up_object_pose_event_term()
    place_drum = place_drum(TargetSide.RIGHT)
...
class EventTargetLeftCfg:
    reset_all = EventTerm(func=mdp.reset_scene_to_default, mode="reset")
    randomize_pick_up_object_pose = randomize_pick_up_object_pose_event_term()
    place_drum = place_drum(TargetSide.LEFT)
...
@configclass
class StickInBinGR1EnvCfg(StickInBinEnvCfg):
    ...
    def __post_init__(self):
        ...
        self.scene.external_cam = CameraCfg(
            ...
            offset=CameraCfg.OffsetCfg(
                pos=[4.6, 1.0, 2.3], rot=[0.9397, 0.342, 0.0, 0.0], convention="opengl"
            ),
        )
