...
from mindmap.diffuser_actor.multihead_custom_attention import MultiheadCustomAttention
from mindmap.diffuser_actor.multihead_flash_attention import MultiheadFlashAttention
...
def convert_multihead_flash_attention(module, process_group=None):
    ...
    for name, child in module.named_children():
        module_output.add_module(name, convert_multihead_flash_attention(child, process_group))
    del module
...
def convert_efficient_fpn(model):
    def _convert_efficient_fpn(module, output_level):
        ...
        if isinstance(module, FeaturePyramidNetwork):
            in_channels_list = [m[0].in_channels for m in module.inner_blocks]
        ...
        for name, child in module.named_children():
            module_output.add_module(name, _convert_efficient_fpn(child, output_level))
        del module
        return module_output

    # Very hackish, requires to know the inner structure of model
    output_level = model.prediction_head.feature_map_pyramid[model.prediction_head.feat_scales - 1]
...
def convert_weights(model: nn.Module):
    ...
    def _convert_weights_to_fp16(l):
        ...
        if isinstance(
            l, (nn.MultiheadAttention, MultiheadCustomAttention, MultiheadFlashAttention)
        ):
            for attr in [
